...
class Graph:
    ...
    def route_farthest_insertion(self) -> "Route":
        """Construit une route heuristique par insertion la plus éloignée."""
        if self.matrice_od is None:
            self.calcul_matrice_cout_od()

        n = self.nb_lieux
        unvisited = set(range(1, n))  # tous les points sauf 0
        tour = [0]

        # Étape 1 : ajouter le point le plus éloigné de 0
        farthest = max(unvisited, key=lambda p: self.matrice_od[0][p])
        tour.append(farthest)
        unvisited.remove(farthest)

        # Étape 2 : boucle principale
        while unvisited:
            # point non visité le plus éloigné du tour actuel
            f = max(unvisited, key=lambda p: min(self.matrice_od[p][t] for t in tour))

            # trouver la position d'insertion qui augmente le moins la distance
            best_pos = None
            best_increase = float('inf')
            for i in range(len(tour)-1):
                a, b = tour[i], tour[i+1]
                increase = self.matrice_od[a][f] + self.matrice_od[f][b] - self.matrice_od[a][b]
                if increase < best_increase:
                    best_increase = increase
                    best_pos = i + 1

            # insertion et mise à jour
            tour.insert(best_pos, f)
            unvisited.remove(f)
    ...
    def route_heuristique(self, methode: str = "ppv") -> "Route":
        ...
        if methode == "ppv":
            ...
            return self.route_farthest_insertion()
