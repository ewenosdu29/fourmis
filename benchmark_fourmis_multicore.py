...
LARGEUR = 1000
HAUTEUR = 800
NB_LIEUX = 10000  # BIG DATA
SEUIL_BIG_DATA = 3000 

# ============================================================
# 2. CLASSE LIEU (IMPOSÉE)
# ============================================================
class Lieu:
...
# ============================================================
# 3. CLASSE GRAPH (OPTIMISÉE GRILLE SPATIALE)
# ============================================================
class Graph:
    def __init__(self):
        self.liste_lieux = []
    ...
    def _calculer_voisins_grille(self, coords, n):
        """Grille spatiale manuelle pour trouver les voisins."""
        t0 = time.time()
        grid_size = 50 
        grid = {}
        ...
        for idx in range(n):
            ...
            final_indices = cands_idx[best_local_indices]
            final_dists = d[best_local_indices]
            
            mask = final_indices != idx
            final_indices = final_indices[mask][:self.k_voisins]
            final_dists = final_dists[mask][:self.k_voisins]
...
# ============================================================
# 5. CLASSE AFFICHAGE (IMPOSÉE + SAMPLING)
# ============================================================
class Affichage:
    def __init__(self, graph, group_name="TSP Project"):
        self.graph = graph
        self.root = tk.Tk()
...
# ============================================================
# 6. CLASSE TSP_ACO (MULTI-THREADING)
# ============================================================
class TSP_ACO:
    def __init__(self, graph, affichage, nb_fourmis=20, alpha=1.0, beta=3.0, rho=0.1):
        ...
        # Verrou pour écriture concurrente
    ...
    def _tache_fourmi(self, seed_offset):
        """La tâche exécutée par un Thread."""
        # On utilise une graine locale pour l'aléatoire dans le thread
        local_random = random.Random(time.time() + seed_offset)
        ...
        for _ in range(self.n - 1):
            ...
            # Filtre Python pur (rapide sur petites listes)
            ...
            if valid_tuple:
                ...
                # Choix rapide
                if local_random.random() < 0.9:
                    # Argmax avec bruit numpy
                    nxt = valid_real[np.argmax(probas * np.random.uniform(0.8, 1.2, size=len(probas)))]
                else:
                    nxt = valid_real[local_random.randint(0, len(valid_real)-1)]
            else:
                # Secours
                while True:
                    cand = local_random.randint(0, self.n-1)
        ...
        # On utilise self.graph.calcul_distance_route qui est thread-safe (lecture seule)
        dist = self.graph.calcul_distance_route(r)
    ...
    def resoudre(self, nb_iter=50):
        ...
        max_workers = 8 # Ajuste selon ton CPU
        print(f"Lancement ACO sur {max_workers} threads...")
        
        for it in range(nb_iter):
            routes = []
            
            # Lancement parallèle des fourmis
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in futures:
                    try:
                        ...
                        if d < best_dist:
                            # Section critique (écriture best_dist) protégée par le GIL
                            # mais on reste prudent
                            best_dist = d
                            best_route = Route(self.graph, o)
                            self.app.log(f"RECORD: {d:.2f}")
                            # Mise à jour GUI doit se faire dans le thread principal idéalement
                            # mais Tkinter tolère parfois, sinon file d'attente.
                            # Ici on met à jour à la fin de l'itération pour la sécurité.
                    except Exception as e:
                        print(f"Erreur thread: {e}")

            # Affichage du meilleur de l'itération
            ...
            for o, d in routes:
                ...
                if d <= best_dist * 1.05:
                    for i in range(self.n):
                        u, v = o[i], o[i+1]
                        # Recherche index (un peu lent mais nécessaire)
                        idx_list = np.where(indices_voisins[u] == v)[0]
                        if idx_list.size > 0: 
                            self.pheromones[u, idx_list[0]] += delta
...
if __name__ == "__main__":
    ...
    # Plus de fourmis car on a des threads !
    solver = TSP_ACO(g, app, nb_fourmis=24)
